...
int uart_lconf(enum UART port, struct ulconf *lconf)
{
	...
	confvalue = confvalue | (lconf->par << 3);
	confvalue = confvalue | (lconf->ired << 6);
	confvalue = confvalue | (lconf->wordlen);
	confvalue = confvalue | (lconf->stopb << 2);
...
int uart_conf_rxmode(enum UART port, enum URxTxMode mode)
{
	...
	switch (port) {
		case UART0:
			...
			if(mode == INT){ic_enable(INT_URXD0);}
			break;
		...
		case UART1:
			...
			if(mode == INT){ic_enable(INT_URXD1);}
			break;
	}
...
static void uart_tx_ready(enum UART port)
{
	switch (port) {
		case UART0:
			...
			while((rUTRSTAT0 & (0x1 << 1)) != 0x2);
			break;

		case UART1:
			//COMPLETAR: esperar a que se vacíe el buffer de transmisión de la
			//uart1 (UTRSTAT1, Transmit Buffer Empty)
			while((rUTRSTAT1 & (0x1 << 1)) != 0x2);
			break;
	}
}
