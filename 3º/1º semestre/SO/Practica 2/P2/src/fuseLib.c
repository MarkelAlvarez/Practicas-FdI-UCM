...
static int my_read(const char *path, char *mem, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	while (bytesParaLeer)
	{
		int i, offsetBloque, bloqueActual;
		...
		{
			fprintf(stderr, "Error al leer los bloques en my_read()\n");
			return -EIO;
		}

		for (i = offsetBloque; (i < BLOCK_SIZE_BYTES) && (totalLeido < size); i++)
		{
			mem[totalLeido++] = buffer[i];
		}

		// Restamos el contenido quitado
		bytesParaLeer -= (i - offsetBloque);
		offset += (i - offsetBloque);
	}
 ...
 *
 * @param path file path
 * @return 0 on success and -ENOENT on error
 **/
static int my_unlink(const char *path)
{
	int i, idxNodoI, nodoI, indBloque, tamBloque;
	...
	// Se actualiza el MapaDeBits
	tamBloque = myFileSystem.nodes[nodoI]->numBlocks;
	for (i = 0; i < tamBloque; ++i)
	{
		indBloque = myFileSystem.nodes[nodoI]->blocks[i];
		myFileSystem.bitMap[indBloque] = 0;
	}

	// Se actualiza el nodoI
	myFileSystem.nodes[nodoI]->fileSize = 0;
	myFileSystem.nodes[nodoI]->numBlocks = 0;
	myFileSystem.nodes[nodoI]->freeNode = true;
	// Se actualiza el FileSystem
	myFileSystem.numFreeNodes++;
	...
	// Se actualiza en nuestro disco virtual, y para ello escribimos en nuestro disco-virtual
	updateSuperBlock(&myFileSystem);
	updateDirectory(&myFileSystem);
	updateBitmap(&myFileSystem);
...
struct fuse_operations myFS_operations = {
	...
	.read = my_read,         // Read data from file
